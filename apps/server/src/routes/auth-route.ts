...
const authRoute: FastifyPluginAsync = async (fastify) => {
  fastify.post<{ Body: { email: string } }>('/api/auth/magic-link', async (request, reply) => {
  ...
  fastify.get<{ Querystring: { token: string; mode?: 'web' | 'token' } }>('/api/auth/verify', async (request, reply) => {
